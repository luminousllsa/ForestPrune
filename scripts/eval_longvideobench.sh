...
if [ "$METHOD" == "baseline" ]; then
    SAVE_DIR=$(basename $CKPT)_${CONV_MODE}_frames_${FRAMES}_${QUESTION_TYPE}_${METHOD}_grid
else
...
if [ "$EVAL_ONLY" == False ]; then
    for IDX in $(seq 1 $CHUNKS); do
        ...
        python3 -W ignore llava/eval/model_$TASK.py \
            --model-path $CKPT \
            --video-dir ./datasets/LongVideoBench/videos \
            --gt-file ./datasets/LongVideoBench/lvb_val.json \
